...
async function createRoadmap(req, res, next) {  
    ...
    const roadmapExists = await roadmapService.checkIfRoadmapExists(req.body.title);
    ...
    if (roadmapExists) {
        ...
        try {
            ...
            if (!newRoadmap) {
                ...
                return  res.status(201).json({ status: "true", message: "roadmap created successfully" });
            }
        } catch (error) {
            console.log(error);
...
async function deleteRoadmap(req, res, next) {
    try {
        ...
        return res.status(200).json({ status: "success", message: "Roadmap deleted successfully" });
    } catch (error) {
        console.log(error);
        return res.status(500).json({ error: "Internal Server Error" });
    }
...
async function updateRoadmap(req, res, next) {
    try {
        ...
        return res.status(200).json({ status: "success", message: "Roadmap updated successfully" });
